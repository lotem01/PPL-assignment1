...
Functional programming improves over object oriented programming by encouraging simpler and more predictable code. Functions usually do one clear task, and we avoid changing shared state, so the code is easier to test, verify, and optimize. This style also works well with asynchronous computations.
